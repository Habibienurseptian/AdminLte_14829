...
@section('content_header')
    <h1 class="text-dark">Tambah Dokter Baru</h1>
@stop
...
<div class="container">
    <div class="card shadow-sm border-0 rounded-4">
        <div class="card-header bg-primary text-white rounded-top-4">
            <h5 class="mb-0">Formulir Pendaftaran Dokter</h5>
        </div>
        <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <select name="poli_id" class="form-select @error('poli_id') is-invalid @enderror" required>
                        ...
                        @foreach ($polis as $poli)
                            <option value="{{ $poli->id }}" {{ old('poli_id') == $poli->id ? 'selected' : '' }}>{{ $poli->nama }}</option>
                        @endforeach
                    </select>
                    @error('poli_id')
                        <div class="invalid-feedback">{{ $message }}</div>
